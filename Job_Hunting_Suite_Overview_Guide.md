...
Four standalone tools cover a full job search, each solving a distinct stage, designed to work together but each fully independent — any one can be used on its own. Two additional tools sit outside that job-search funnel entirely: **LinkedIn Article Share Builder**, for staying visible on LinkedIn whether or not you're actively job hunting, and **Recruiter Message Sanity Check**, for screening suspicious recruiter outreach — both covered separately near the end of this guide. This guide covers **what each tool does and how they fit together**. For **how to actually use each one** — every field, every toggle, every output option — see that tool's own User Guide or Sale Fish combined guide, linked at the bottom.
...
|---|---|---|---|
| **Target Company Prompt Builder** | Researches and ranks companies — builds a formatted tracker with a computed Suggested Priority Rank | A company, industry, or NAICS code you already know | Prompt text — paste into a new Claude chat to generate the `.xlsx` tracker |
| **Job Posting Finder** | Checks who's actively hiring, right now, with real postings and real URLs | A shortlist of companies and/or job titles | Prompt text — paste into a new Claude chat to generate the postings list |
| **Resume & Cover Letter Tailoring** | Tailors your actual application to a specific posting — gap analysis, bullet rewrites, cover letter or Qualifications Match Letter | A real job posting's text, plus your resume | Prompt text — paste into a new Claude chat to generate the tailored documents |
| **Outreach Message Builder** | Drafts the messages you send to actually reach someone | Company, contact, and connection-type data from your tracker | Prompt text — paste into a new Claude chat to generate the draft messages |

Note the **Output** column above: none of these four tools produce a finished document by themselves. Each one builds a prompt that you copy into a separate Claude conversation, where the actual research, writing, or file generation happens. That's a deliberate design choice, not a missing feature — see "What's Consistent Across All Four Tools" below.
...
| LinkedIn Article Share Builder | Required — the entire tool depends on live search for real articles | Only if you choose to generate matching artwork |
| Recruiter Message Sanity Check | Not required | Not required — the tool runs entirely in your browser; no Claude chat needed at all |
...
| Recruiter Message Sanity Check | `Recruiter_Message_Sanity_Check_User_Guide.md` / `.docx` | `Sale_Fish_Recruiter_Message_Sanity_Check_Combined_Guide.docx` |
...
Like the four funnel tools, this one generates a prompt for a separate Claude conversation — it doesn't draft the post itself.
...
## A Sixth Tool, Also Outside the Funnel: Recruiter Message Sanity Check

Fake recruiter messages on LinkedIn and email have become common enough to warrant their own tool. This one is different in kind from every other tool in this suite: paste in a suspicious message, optionally note the claimed company and sender's email, and check off any red flags visible on the sender's profile — the tool scores the message against a fixed list of known scam patterns and hands back a verdict (Low / Caution / High) with every flagged phrase highlighted in place.

Unlike the five tools above, **this one does not generate a prompt for a separate Claude conversation.** It's a self-contained scanner — the entire check runs in your browser, and the finished result appears immediately. No Claude chat, no copy-paste step, no Claude Settings requirements at all.

It carries the same honesty discipline as the rest of the suite, adapted to what it produces: the score is explicitly labeled "pattern match strength," not a statistical probability of fraud, since no dataset exists to calculate a true probability honestly. A clean result never implies a message is confirmed legitimate, and a flagged one is never treated as proof of a scam — both push the user back toward verifying independently.
